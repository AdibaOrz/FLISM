...
class MOON_Client(object):
    ...
    def get_communication_cost(self):  # Local training
        download_model_lists = [self.model, self.model.encoder, self.model.projection_head]
        upload_model_lists = [self.model]
        download_time_global_model = self.get_download_time(download_model_lists)
        upload_time_global_model = self.get_upload_time(upload_model_lists)

        local_communication_cost = download_time_global_model + upload_time_global_model

        local_number_of_trained_parameters = 0
