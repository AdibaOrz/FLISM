"""
Harmony Models for PAMAP2 dataset
"""
import torch
...
class Dynamic_Encoder(nn.Module):
    def __init__(self, modality_indices, device, debug=False):
        ...
        self.total_num_modalities = PAMAP2_Opt['num_modalities']
        ...
        if self.total_num_modalities == 3: # 3 positions in the case of PAMAP2 dataset
    ...
    def forward(self, x):
        ...
        for idx, is_available in enumerate(self.modality_indices):
            if is_available:
                ...
                input_data = get_modality_input_pamap2(x, idx, self.modality_type)
                if self.debug:
...
def get_modality_input_pamap2(input_data, modality_index, modality_type):
    if modality_type == "acc_gyro_6axes":
        return input_data[:, modality_index*6:(modality_index+1)*6, :]
    elif modality_type == "acc_or_gyro_3axes":
        return input_data[:, modality_index*3:(modality_index+1)*3, :]
    else:
        raise ValueError(f"Invalid modality type: {modality_type}")
