...
class Simulated_Dynamic_Model(nn.Module):
    def __init__(self, args):
        super(Simulated_Dynamic_Model, self).__init__()

        self.num_modalities = args.simulated_number_of_modalities
        dim_lists = [[j + i for j in range(1)] for i in range(self.num_modalities)]
        self.encoder = Dynamic_Encoder(self.num_modalities, dim_lists)
        self.classifier = Classifier(Simulated_Opt['num_classes'])
...
class Single_Encoder(nn.Module):
    def __init__(self, input_channel_dim):
        ...
        self.encoder = nn.Sequential(
            nn.Conv1d(input_channel_dim, 32, kernel_size=4, stride=1),
            nn.ReLU(True),
            nn.Dropout(0.1),
