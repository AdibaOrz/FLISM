...
=> Apply FedProx to solve heterogeneity between the local and global models
"""
import copy
...
class FedProx_Client(object):
    ...
    def local_train(self, model):
        ...
        prev_global_model = copy.deepcopy(model)
        model.train()
        ...
        for local_epoch in range(local_epochs):
            ...
            for batch_idx, (input_data, labels) in enumerate(self.train_loader):
                ...
                proximal_term = 0.0
                for w, w_t in zip(model.parameters(), prev_global_model.parameters()):
                    proximal_term += (w - w_t).norm(2)

                cls_loss = self.criterion(model_outputs, labels)
                prox_loss = (self.args.prox_mu / 2) * proximal_term
