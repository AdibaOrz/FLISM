...
=> Apply MOON method to handle missing modalities.
"""
import copy
...
class MOON_Client(object):
    ...
    def local_train(self, model, prev_local_model):
        model, prev_local_model = model.to(self.device), prev_local_model.to(self.device)
        global_model = copy.deepcopy(model)

        model.train()
        prev_local_model.eval() # Negative
        global_model.eval() # Positive

        epoch_loss = []
        local_epochs = self.args.local_ep
        self.optimizer = self.get_optimizer(model)

        cos = torch.nn.CosineSimilarity(dim=-1).to(self.device)
        ...
        for local_epoch in range(local_epochs):
            ...
            for batch_idx, (input_data, labels) in enumerate(self.train_loader):
                ...
                # MOON
                feature_local = model(input_data, extract_embeds_only=True)
                feature_pos = global_model(input_data, extract_embeds_only=True)

                posi = cos(feature_local, feature_pos)
                logits = posi.reshape(-1, 1)

                feature_neg = prev_local_model(input_data, extract_embeds_only=True)
                nega = cos(feature_local, feature_neg)
                logits = torch.cat((logits, nega.reshape(-1, 1)), dim=1)

                logits /= self.args.moon_temp
                targets = torch.zeros(input_data.size(0)).to(self.device).long()

                loss_moon = self.args.moon_mu * self.criterion(logits, targets)

                # Classification
                model_outputs = model(input_data, extract_embeds_only=False)
                loss_cls = self.criterion(model_outputs, labels)

                loss = loss_cls + loss_moon
                loss.backward()
                self.optimizer.step()
