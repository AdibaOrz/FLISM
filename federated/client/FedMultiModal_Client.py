    ...
    def local_train(self, model):
        ...
        for local_epoch in range(local_epochs):
            ...
            for batch_idx, (input_data, labels) in enumerate(self.train_loader):
                ...
                # Separate the input data per modalities and put it into the dynamic encoders
                dim_lists = self.args.dataset_opts['dim_lists']
                num_modalities = self.args.dataset_opts['num_modalities']
                separated_input_list = [input_data[:, dim_lists[i], :] for i in range(num_modalities)]

                model_outputs = model(separated_input_list)
    ...
    def local_test(self, model):
        ...
        with torch.no_grad():
            for batch_idx, (input_data, labels) in enumerate(self.test_loader):
                ...
                outputs = model(separated_input_list)
                labels = labels.long()
