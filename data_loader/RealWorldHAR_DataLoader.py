...
from configs import RealWorldHAR_Opt

sys.path.append('../')
WIN_LEN = RealWorldHAR_Opt['window_len']

class RealWorldHAR_Dataloader(torch.utils.data.Dataset):
    ...
    def get_column_name_index_dict(self):
        ...
        for col_idx, col_name in enumerate(column_list):
            if col_name in ['user_idx', 'label', 'index']:
                dict[col_name] = col_idx
        dict['feat_start'] = dict['index'] + 1
        dict['feat_end'] = dict['label'] - 1
        return dict
